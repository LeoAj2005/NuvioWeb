...
export function renderLegacySidebar({
  ...
  const themeColor = profileState.activeProfileColorHex || getThemeAccentFallback();
  ...
  return `
    <aside class="home-sidebar root-sidebar root-sidebar-legacy${performanceConstrained ? " performance-constrained" : ""}"
      ...
      <div class="sidebar-brand-header" style="display: flex; justify-content: center; padding: 24px 0 16px 0;">
        <img src="assets/images/largeIcon.png" alt="App Logo" width="100" height="100" style="object-fit: contain; max-width: 100%; height: auto;" />
      </div>

      <div class="home-nav-list" style="flex: 1; overflow-y: auto;">
        ${ROOT_SIDEBAR_ITEMS.map((item) => `
          <button class="home-nav-item focusable${selectedItem.action === item.action ? " selected" : ""}"
                  data-action="${item.action}"
                  aria-label="${itemLabel(item)}"
                  ${selectedItem.action === item.action ? `style="color: ${themeColor};"` : ""}>
      ...
      ${showProfileSelector ? `
        <div class="sidebar-footer" style="padding-top: 16px; margin-top: auto; padding-bottom: 24px;">
          <button class="home-profile-pill focusable"
                  data-action="gotoAccount"
                  aria-label="${t("sidebar.switchProfile")}">
            <span class="home-profile-avatar" style="background:${themeColor}">
...
export function renderModernSidebar({
  ...
  return `
    <div class="modern-sidebar-shell${expanded ? " expanded" : ""}${blurEnabled ? " blur-enabled" : ""}${keepPillExpanded ? " keep-pill-expanded" : ""}${performanceConstrained ? " performance-constrained" : ""}" data-selected-route="${selectedRoute}">
      ...
      ` : ""}
      <aside class="modern-sidebar-panel" aria-hidden="${expanded ? "false" : "true"}"${expanded ? "" : " hidden"} style="display: flex; flex-direction: column; height: 100%;">
        
        <div class="modern-sidebar-brand" style="display: flex; align-items: center; justify-content: center; padding: 24px 16px;">
          <img src="assets/images/largeIcon.png" alt="App Logo" width="100" height="100" style="object-fit: contain; max-width: 100%;" />
        </div>

        <div class="modern-sidebar-nav-list" style="flex: 1; overflow-y: auto;">
          ${ROOT_SIDEBAR_ITEMS.map((item) => `
            <button class="modern-sidebar-nav-item focusable${selectedItem.action === item.action ? " selected" : ""}"
                    data-action="${item.action}"
                    aria-label="${itemLabel(item)}">
              <span class="modern-sidebar-nav-icon-circle" ${selectedItem.action === item.action ? `style="background-color: ${themeColor}; color: #fff;"` : ""}>
                ${iconMarkup(item, "modern-sidebar-nav-icon")}
              </span>
              <span class="modern-sidebar-nav-label" ${selectedItem.action === item.action ? `style="color: ${themeColor}; font-weight: 600;"` : ""}>${itemLabel(item)}</span>
            </button>
          `).join("")}
        </div>

        ${showProfileSelector ? `
          <div class="modern-sidebar-footer" style="padding: 16px; margin-top: auto; border-top: 1px solid rgba(255,255,255,0.1);">
            <button class="modern-sidebar-profile focusable" data-action="gotoAccount" aria-label="${t("sidebar.switchProfile")}">
              <span class="modern-sidebar-profile-avatar" style="background:${themeColor}">
                ${profileState.activeProfileAvatarUrl
                  ? `<img class="sidebar-profile-avatar-image" src="${profileState.activeProfileAvatarUrl}" alt="${profileState.activeProfileName || t("sidebar.profileFallback")}" />`
                  : (profileState.activeProfileInitial || "P")}
              </span>
